...
interface RegisterFormProps {
  error?: ZodError<z.infer<typeof RegisterDataSchema>>;
  data?: {
    email: string;
    username: string;
    password: string;
...
export const RegisterForm = component$<RegisterFormProps>(({ error, data }) => {
  return (
    <form
    ...
    >
      <div>
        ...
        <p class="mb-4 text-error h-4">
          {" "}
          {
            error?.errors.find((error) => error.path.join(".") === "email")
              ?.message
          }
        </p>
      </div>
      ...
      <div>
        ...
        <p class="mb-4 text-error h-4">
          {" "}
          {
            error?.errors.find((error) => error.path.join(".") === "username")
              ?.message
          }
        </p>
      </div>
      ...
      <div>
        ...
        <p class="mb-4 text-error h-4">
          {" "}
          {
            error?.errors.find((error) => error.path.join(".") === "password")
              ?.message
          }
        </p>
      </div>
...
export const RegisterSuccess = component$(() => {
  return (
    <div class="card mx-auto w-96 bg-base-100 shadow-xl">
      <h1 class="text-2xl">Thank you for registering</h1>
